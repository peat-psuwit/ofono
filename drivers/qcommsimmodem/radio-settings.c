...
static void qcom_msim_set_2g_rat_mode_cb(struct ril_msg *message,
							gpointer user_data)
{
	struct ofono_radio_settings *rs = user_data;
	struct qcom_msim_radio_data *rd =
				ofono_radio_settings_get_data(rs);
	...
	if (message->error == RIL_E_SUCCESS) {
		...
		ofono_error("%s: rat mode setting to GSM failed", __func__,
							pending_pref_slot);
	}

	if (pending_pref_slot < 0) {
		/* The pending pref setting has been canceled */
		return;
	}
	...
	if (pref == PREF_NET_TYPE_GSM_WCDMA && multisim_rs_amount > 1) {
		...
		for (i = 0; i < MULTISIM_RS_LAST; i++) {
			if (multisim_rs[i] == rs) {
				...
				rd->pending_pref_wait_amount =
							multisim_rs_amount - 1;

				pending_pref_slot = i;
			} else if (multisim_rs[i] != NULL) {
				struct qcom_msim_radio_data *rd =
						ofono_radio_settings_get_data(
						multisim_rs[i]);
				g_ril_request_set_preferred_network_type(
					...
					unsigned int vendor, void *user)
{
	...
	struct qcom_msim_radio_data *rsd = g_try_new0(
					struct qcom_msim_radio_data, 1);
	int slot_id;
