...
static int sim_select_uicc_subscription(struct ofono_sim *sim, struct reply_sim_status *status)
{
	...
	if (selected_app != -1)
		...
		sim_send_set_uicc_subscription(sim, slot_id, selected_app, slot_id, 1);
	...
	if (status->card_state == RIL_CARDSTATE_PRESENT) {
		...
		if (search_index < status->num_apps) {
			...
			if (app->app_type != RIL_APPTYPE_UNKNOWN) {
				 ...
				 * this should be done by issuing a GET_SIM_STATUS
				 * request from ril_query_passwd_state, which
				 * is called by the core after sending a password,
				 * but unfortunately the response to GET_SIM_STATUS
				 * is not reliable in mako when sent just after
				 * sending the password. Some time is needed before
				 * the modem refreshes its internal state, and
				 * when it does it sends a SIM_STATUS_CHANGED
				 * event. In that moment we retrieve the status
				 * and this function is executed. We call
				 * __ofono_sim_recheck_pin as it is the only way
				 * to indicate the core to call query_passwd_state
