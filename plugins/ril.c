...
{
	struct ril_data *rd = ofono_modem_get_data(modem);
	int slot_num = ofono_modem_get_integer(modem, "Slot");

	rd->ril = g_ril_new(RILD_CMD_SOCKET[slot_num], OFONO_RIL_VENDOR_AOSP);
	...
	g_ril_set_slot(rd->ril, slot_num);

	if (getenv("OFONO_RIL_TRACE"))
		g_ril_set_trace(rd->ril, TRUE);

	if (getenv("OFONO_RIL_HEX_TRACE"))
		g_ril_set_debugf(rd->ril, ril_debug, GRIL_DEBUG_HEX_PREFIX[slot_num]);
